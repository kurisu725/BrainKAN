...
class KAN(torch.nn.Module):
    ...
    def plot(self, folder="./figures", beta=3, mask=False, mode="supervised", scale=0.5, tick=False, sample=False,
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        # matplotlib.use('Agg')
        depth = len(self.layers_hidden) - 1
        for l in range(depth):
            w_large = 2.0
            for i in range(self.layers_hidden[l]):
                for j in range(self.layers_hidden[l + 1]):
                    ...
                    numerical_mask = self.act_fun[l].mask.reshape(self.layers_hidden[l + 1], self.layers_hidden[l])[j][i]
                    ...
                    lock_id = self.act_fun[l].lock_id[j * self.layers_hidden[l] + i].long().item()
                    if lock_id > 0:
